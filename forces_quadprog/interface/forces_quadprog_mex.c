/*
 This is an interface for forces_quadprog that is used by optimizerFORCES to call the solver
*/ 
...
forces_quadprog_params params;
forces_quadprog_output output;
forces_quadprog_info info;
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
	...
	if( !mxIsCell(param_values) ) {
		mexErrMsgTxt("forces_quadprog requires a cell array as input.");
	}
	...
	#if SET_PRINTLEVEL_forces_quadprog > 0
	...
	/* call solver */
	exitflag = forces_quadprog_solve(&params, &output, &info, fp );
	...
	copyCArrayToM(output.output1, mxGetPr(plhs[0]), 2);
