...
est = [T_hat; d_hat(:,end)];
x0_est = T_hat-T_sp;
u_est = yalmip_optimizer([x0_est, d_hat(:,end), Ucons, Xcons]); %estimate control input
p_est = u_est+p_sp;
...
% get optimal u
[u, errorcode] = yalmip_optimizer([x0, d_hat(:,end), Ucons, Xcons]);
p = u + p_sp;
% Analyze error flags
if (errorcode ~= 0)
...
yalmip_opt = optimizer(constraints,objective,ops,[x0, d0, Ucons, Xcons],U{1});
end