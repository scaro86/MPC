...
% from the command line. It also illustrates how to use the FORCES PRO
% plugin for the MPC toolbox. 
%
% This example requires Model Predictive Control Toolbox from MathWorks.

% Copyright 2019 The MathWorks, Inc. and Embotech AG, Zurich, Switzerland
...
% Create options structure for the FORCES PRO sparse QP solver. This uses the
% multi-stage formulation of the QP problem.
forcesOptions = mpcToForcesOptions('sparse');
% Generates the FORCES PRO solver
...
if ~mpcchecktoolboxinstalled('matlabcoder')
	error('Error. Coder needs to be installed in order to generate c code from the FORCES PRO plugin.');
end
