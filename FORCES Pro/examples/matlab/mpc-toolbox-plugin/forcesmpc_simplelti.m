...
% the FORCES PRO plugin for the mpc toolbox
...
%% Instantiate FORCES PRO options using Sparse QP formulation
options = mpcToForcesOptions('sparse');

%% Generate FORCES PRO solver code from MPC object
...
for k = 1:SimuSteps
    ...
    if info.ExitFlag < 0
        warning('Internal problem in FORCES PRO QP solver');
    end
    U(:,k) = mv;
...
%% Instantiate FORCES PRO options using Dense QP formulation
