...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'ControlToOrigin_solver/lib/ControlToOrigin_solver.dll')) 
try:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'ControlToOrigin_solver/lib/ControlToOrigin_solver.dll'))
	csolver = getattr(_lib,'ControlToOrigin_solver_solve')
except:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'ControlToOrigin_solver/lib/libControlToOrigin_solver.dll'))
	csolver = getattr(_lib,'ControlToOrigin_solver_solve')
...
a Python wrapper for a fast solver generated by FORCES PRO v3.0.0
 ...
 See also COPYING
	...
	if sys.version_info.major == 2:
		...
		try:
			PyFile_AsFile.restype = ctypes.POINTER(FILE)
			exitflag = _lib.ControlToOrigin_solver_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp)  )
		...
		try:
			if sys.platform.startswith('win'):
				exitflag = _lib.ControlToOrigin_solver_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), None )
			else:
				exitflag = _lib.ControlToOrigin_solver_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), fp )
			libc.fclose(fp)
